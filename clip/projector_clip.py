...
#         images = G.synthesis(
#             torch.tensor(latents[i * batch_size:(i + 1) * batch_size, :, :], dtype=torch.float32, device=device),
#             noise_mode='const')
#         with torch.no_grad():
...
#                     ((target_images / 255.0) - image_mean[None, :, None, None]) / image_std[None, :, None,
#                                                                                   None]).float()
#                 if use_center:
#                     clip_target_center = model.encode_image(
#                         ((center_target / 255.0) - image_mean[None, :, None, None]) / image_std[None, :, None,
#                                                                                       None]).float()
#
#     if kmeans_latents is not None and use_clip and target_text is not None:
#         scores, kmeans_images = score_images(G, model, target_text, kmeans_latents.repeat([1, G.mapping.num_ws, 1]),
#                                              device=device)
...
#
#         if use_clip:
#             clip_synth_image = ((synth_images / 255.0) - image_mean[None, :, None, None]) / image_std[None, :, None,
#                                                                                             None]
#             clip_synth_features = model.encode_image(clip_synth_image).float()
#             adj_center = 2.0
#
#             if use_center:
#                 clip_cynth_center_image = ((center_synth / 255.0) - image_mean[None, :, None, None]) / image_std[None,
#                                                                                                        :, None, None]
#                 adj_center = 1.0
